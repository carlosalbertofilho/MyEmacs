...
> **Workflow:** Work in `~/Projects/Github/MyEmacs` → commit/push → sync to `~/.config/emacs-vanilla` → test.
...
# In ~/.config/emacs-vanilla (test environment)
...
```
~/.config/emacs-vanilla/          ← EMACSDIR
├── early-init.el                 ← Startup optimizations
├── init.el                       ← Bootstrap: package archives, load-path, require modules
...
;; External packages: :ensure t (default via use-package-always-ensure)
...
- **Package sources:** GNU ELPA, NonGNU ELPA, MELPA, Nix site-lisp
- **Key packages installed via Nix:** emacs-jupyter, pdf-tools (partial), nerd-icons fonts
...
# Batch test
emacs --init-directory ~/.config/emacs-vanilla --batch -l init.el --eval '(message "OK")'
```
