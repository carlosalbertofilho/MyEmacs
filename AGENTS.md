...
- **2026-08-27/28:** 2 testes `magent-*` (`myemacs-magent-subagent-override-consumed` e `myemacs-magent-subagent-apply-profile-unknown-agent`) falham com `wrong-type-argument gptel-backend` — introduzida pelo commit `f893090` (coder-lite) que chama `gptel-backend-name` incondicional em `+carlos/magent-subagent-apply-profile` quando o backend é plist `(:name ...)` ou string `local`. Correção em andamento: helper `+carlos/--magent-backend-name` criado, mas o ERT ainda persiste (investigar definições duplicadas de `apply-profile` / `gptel-backend-name` no load-path completo).
