...
| Coder Lite zerava o toolset de modelos locais: o filtro `+carlos/magent-subagent-apply-profile` comparava **nomes de tool** com underscore (`read_file`, `run_command`, `replace_file_content`) contra `magent-enable-tools`, que guarda **permission keys curtas** (`read`→`read_file`, `bash`→`run_command`, `write`→`write_file`) → interseção vazia e modelo local sem ler/escrever/executar | `+carlos/magent-subagent-lite-tools` (defcustom, `lisp/custom-magent-subagent.el`): filtro lite agora usa permission keys (`read`/`write`/`edit`/`bash` + `context_search` + `*_smart_edit`). Verificado em batch: n=12 tools (read_file/read_buffer + smart edits) chegam ao gptel. Blocker F7-ben remanescente (2026-08-30, refinado por probe direto `magent-llm-gptel-sample`): `gemma-4-e2b-it-4bit` e `Qwen3.5-9B-MLX-4bit` emitem `tool-call` com payload pequeno; no run completo o request grande → ~0 bytes no stream (4 chunks/320B em 60s) → watchdog `magent-request-timeout` (120s) aborta antes de qualquer evento (probe: `FSM-LOG=(abort)`, callback `nil`). Próximo passo: compactar system prompt/lite toolset OU elevar timeout p/ backend local | `myemacs-magent-subagent-lite-tools-native-permission-keys` + `myemacs-magent-subagent-lite-filter-keeps-native` (tests/magent-subagent-test.el) + probe `magent-enable-tools` batch |
