...
** Plano de Ação — Fase D+: Evolução da Orquestração e Equipe de Especialistas
...
*** TODO D4. Codificar Ferramentas do Perfil `coder` (Operador de Buffer Vivo)
    ...
    :TAGS: :magent:tools:
    :END:
    Implementar as mutações `buffer_insert`, `buffer_replace_region`, `buffer_undo` e sensores `lsp_hover`, `describe_elisp_symbol`. (Ver docs/magent-reference.org para a arquitetura completa).
...
*** TODO D5. Context Sharing Dinâmico para Subagentes
    ...
    :STATUS: backlog
    :END:
    Injetar automaticamente contexto relevante nos prompts enviados a subagentes (`spawn_agent` aprimorado).
...
*** TODO D6. Feedback de UI Visual para Delegação (Fase C+)
    ...
    :STATUS: backlog
    :END:
    Enriquecer o painel `*Magent*` com spinners enquanto a FSM bloqueia em `subagent-waiting`.
...
*** TODO D9. Instanciar a Equipe de Especialistas (Sysadmin, Planner, Tech Writer, Auditor, Sec-Ops, QA)
    ...
    :STATUS: backlog
    :END:
    Criar o prompt base de cada subagente em Lisp conforme as especificações arquiteturais (movidas para `docs/magent-reference.org`).
...
*** TODO D10. Integrar Pacote `rfc-mode` e Ferramentas Lisp
    ...
    :END:
    Adicionar `rfc-mode` no `init.el` (via Elpaca) e criar as ferramentas Lisp `rfc_search_topic` e `rfc_read_section` no `custom-magent-tools.el` para equipar o Auditor e o Sec-Ops.
