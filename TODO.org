...
- **D9. Criação da Equipe de Especialistas (Expansão de Perfis Futuros):**
  ...
  - *Perfil `librarian` (ou `tech-writer`):* Engenheiro de Conhecimento e Guardião do RAG. Mantém a pasta `docs/` como Fonte Única de Verdade.
    - *Fábrica de RAG via MarkItDown:* A ferramenta `rag_convert_document` padroniza o consumo de fontes externas ricas (PDFs, PPTXs, HTMLs). Ela usa a biblioteca `markitdown` (Microsoft) para extração limpa para Markdown e, em seguida, injeta na estrutura semântica nativa do Org-mode, criando RAG indexável de alta qualidade.
    - *Sincronização Código ↔ Docs:* A ferramenta `rag_sync_api` monitora as alterações feitas pelo Coder, extrai as novas *docstrings* via AST e atualiza cirurgicamente a documentação de referência em `docs/`, garantindo que IAs futuras não alucinem baseadas em código stale.
    - *Integridade Zettelkasten:* Especialista no **Denote**, cruza *backlinks* e usa `rag_verify_integrity` para caçar arquivos que citam funções deletadas pelo Archeologist, mantendo a documentação viva e coerente.
  ...
  - *Perfil `auditor` (ou `staff-engineer`):* Revisor de Arquitetura e Guardião dos Padrões (SOLID e Norminette).
    - *O Carrasco das Linhas:* A ferramenta `treesit_count_executable_lines` usa a AST do tree-sitter para impor um limite rígido de 25 linhas executáveis (C/Norminette da Ecole 42) ou 30 linhas (outras linguagens), barrando funções monstruosas e forçando modularidade.
    - *Conformidade com RFCs:* A ferramenta `read_rfc_spec` permite que o Auditor consulte as especificações técnicas oficiais (ex: OAuth 2.0, JWT) da IETF. Ele barra planos que inventem "o que funciona" em vez de seguir o padrão da indústria.
    - *Auditoria de Complexidade:* Rejeita funções com mais de 4 níveis de indentação ou violações óbvias do Princípio da Responsabilidade Única (SOLID).
  - *Perfil `sec-ops` (ou `red-team`):* Especialista em Segurança, OWASP e Criptografia.
    - *Leitor de "Security Considerations":* Orientado a focar estritamente na seção de segurança das RFCs ao validar um plano de implementação do Planner (ex: checar proteções de Origin Check em WebSockets).
    - *OWASP Top 10 Nativo:* Ferramentas de análise profunda para barrar injeção de comandos (bash gerado pelo agente), Path Traversal (manipulação maliciosa de TRAMP) e XSS.
    - *Policiamento Criptográfico:* Impede a aprovação de PRs/planos que utilizem algoritmos obsoletos (MD5, SHA1) ou que façam *hardcode* de senhas, exigindo uso do Agenix ou do padrão NIST/FIPS.
